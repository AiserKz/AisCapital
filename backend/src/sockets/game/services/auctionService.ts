...
export const endAuction = async (
    ...
    if (auction.currentBidder && auction.currentBid > 0) {
        ...
        if (
            ...
            console.log(`❌ Клетка ${targetCell?.name} не может быть куплена на аукционе`);
            sendRoomMessage(
                io,
                room.id,
                "",
        ...
        const existingCell = cellState.find((c: any) => c.id === auction.cellId);
        if (existingCell && existingCell.ownerId) {
            console.log(`❌ Клетка ${targetCell.name} уже принадлежит другому игроку`);
        ...
        } = await import("./monopolyService.js");
        ...
        if (cellColor) {
            ...
            if (playerHasMonopoly) {
                console.log(`🎯 ${winner.player.name} получил монополию на ${cellColor}!`);

                // Обновляем ренты для всех клеток этого цвета
                updatedCellState = updatedCellState.map((cell) => {
                    const cellColorCheck = getCellColor(cell.id);

                    if (cellColorCheck === cellColor && cell.ownerId === auction.currentBidder) {
